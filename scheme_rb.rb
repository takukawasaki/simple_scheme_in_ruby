...
$global_env = [$list_environment, $function_environment, $boolean_env, $debug_env]
...
def parse(exp)
  program = macro_quotation(exp.gsub(/\n/, ' ').strip()).
...
def closure_to_exp(closure)
  [closure[1], closure[2], closure[3]]
end

def lambda_apply(closure, args)
  parameters, body, env = closure_to_exp(closure)
...
def define_funcname_param_val(exp)
  var = car(exp[1])
...
def eval_define(exp, env)
  if define_with_parameter?(exp)
    var, val = define_funcname_param_val(exp)
  else
...
def _apply_primitive_fun(fun_symbol, args)
  apply_primitive_fun($function_environment[fun_symbol], args)
end
...
def pp(exp)
  if exp.is_a?(Symbol) or num?(exp)
    ...
    if exp == $function_environment
      '*prinmitive_fun_env*'
    elsif exp == $boolean_env
      '*boolean_env*'
    elsif exp == $list_environment
      '*list_env*'
    else
